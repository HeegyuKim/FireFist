        ...
        , DataApi.DataListener
{
        ...
        results.setResultCallback(new ResultCallback<DataItemBuffer>() {
            ...
            public void onResult(DataItemBuffer dataItems) {
                ...
                if(dataItems.getCount() > 0)
                {
                    ...
                    {
                        DataMapItem mapItem = DataMapItem.fromDataItem(dataItems.get(i));
                        if(mapItem.getUri().toString().endsWith("/player"))
                        {
                            Log.d("DataItem", "Find exists item " + mapItem.getUri().toString());
                            putData(mapItem.getDataMap());
                            hasUserMap = true;

                            PutDataMapRequest req =
                                    PutDataMapRequest.createFromDataMapItem(mapItem);
                            Wearable.DataApi.putDataItem(client, req.asPutDataRequest());
                            break;
                        }
                    }
                }
                // 데이터 맵이 없어서 추가해야 할 경우
                if(!hasUserMap)
                {
                    Log.d("DataItem", "Put New DataItem");
                    PutDataMapRequest dataMap = PutDataMapRequest.create("/player");
                    putData(dataMap.getDataMap());
                    PutDataRequest req = dataMap.asPutDataRequest();
                    Wearable.DataApi.putDataItem(client, req);
                }
                dataItems.release();
            }
        });
    ...
    private void putData(DataMap map) {
        Log.d("PUT_DATA", "Data put, total score: " + totalScore);
        map.putFloat("max_score", maxScore);
        map.putFloat("total_score", totalScore);
        map.putFloat("exp", exp);
        map.putInt("level", level);
        map.putInt("coin", coin);
    }
